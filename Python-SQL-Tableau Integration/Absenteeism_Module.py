...
##  creating the special class that we are going to use to predict new data
class absenteeism_model():
      
        def __init__(self, model_file, scaler_file):
            # load the 'model' and 'scaler' files which were saved from Absenteesim Ml section
            with open('model','rb') as model_file, open('scaler', 'rb') as scaler_file:
                self.reg = pickle.load(model_file)
                self.scaler = pickle.load(scaler_file)
                self.data = None
        
        # take a new data file (*.csv) and preprocess it in the same way as in Absenteesim preprocessing section
        def load_and_clean_data(self, data_file):
            ...
            # to preserve the code we've created in the Absenteesim Ml section, we will add a column with 'NaN' strings
            ...
            # drop the variables we decided we don't need after exploring the weights
            df = df.drop(['Day of the Week','Daily Work Load Average','Distance to Work'],axis=1)
            
            # copy the preprocessed data in a new variable
            self.preprocessed_data = df.copy()
            
            # use the scaler object to scale the inputs(non dummy variables)
